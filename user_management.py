...
def insertUser( username, password, securityQ1, securityQ2, securityQ3, securityA1, securityA2, securityA3, address, phoneNumber, pfp, last_attempt:float): #will have to change with hashing algorithm
    con = sql.connect(database_dir)
    cur = con.cursor()
    cur.execute(
        "INSERT INTO users (username, password, securityQ1, securityQ2, securityQ3, securityA1, securityA2, securityA3, address, phoneNumber, picture, role, attempts, last_attempt) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        (username, password, securityQ1, securityQ2, securityQ3, securityA1, securityA2, securityA3, address, phoneNumber, pfp, 2, 5, last_attempt)
    )
...
def get_displayables(username:str) -> list:
    ...
    cur = con.cursor()
    res = cur.execute(
        "SELECT securityQ1, securityQ2, securityQ3, address, phoneNumber, picture FROM users WHERE username=?;", (username, )
    )
    user = res.fetchone()
    con.close() 
